...
// findForms finds all HTML forms in the given string and returns
// a slice of strings containing the forms.
func findForms(html string) []string {
	var forms []string

	// Split the HTML string by the "<form" string to get a slice of
	// strings containing all forms in the page
	formStrs := strings.Split(html, "<form")

	// Iterate over the slice of form strings and extract the forms
	for _, formStr := range formStrs {
		if strings.Contains(formStr, "</form>") {
			// If the form string contains the "</form>" string, then it is a valid
