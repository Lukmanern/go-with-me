...
// It has a field of type Service that will be injected with a concrete implementation.
type Controller struct {
    	Service Service
}

// The DoSomething method of the Controller struct uses the Service interface
// to perform some operation and return the result as a string.
...
func main() {
	...
	// Inject the RealService as the service to be used by the controller.
	controller := &Controller{Service: &RealService{}}

	// Call the DoSomething method of the Controller struct.
	// It will use the injected RealService to perform the operation.
	result := controller.DoSomething()
	fmt.Println(result) // Output: "Using real service"

	// Inject the FakeService as the service to be used by the controller.
	controller.Service = &FakeService{}

	// Call the DoSomething method of the Controller struct again.
	// This time, it will use the injected FakeService to perform the operation.
	result = controller.DoSomething()
	fmt.Println(result) // Output: "Using fake service"
}
